#ifndef ___Tornado_XEffects__EffectHandler__H___
#define ___Tornado_XEffects__EffectHandler__H___
	...
	namespace XEffects
	{
		...
		public:
			...
			*/
			...
			/// Delete all shadow lights
			inline void removeAllShadowLights(void)
			{
				LightList.clear();
			}

			/// Delete a shadow light
			inline void removeShadowLight(irr::u32 index)
			{
				LightList.erase(index);
			}

			/// Delete all shadow nodes
			inline void removeAllShadowNodes(void)
			{
				ShadowNodeArray.clear();
			}

			__TORNADO_DLL_EXPORT void removeShadowLightById(irr::u32 id);

			/// Adds a shadow light. Check out the shadow light constructor for more information.
			inline void addShadowLight(const SShadowLight& shadowLight)
			{
			...
			inline SShadowLight& getShadowLight(irr::u32 index)
			{
				return LightList[index];
			}

			__TORNADO_DLL_EXPORT SShadowLight* getShadowLightById(irr::u32 id);

			/// Retrieves the current number of shadow lights.
			inline const irr::u32 getShadowLightCount() const
			{
			...
			__TORNADO_DLL_EXPORT irr::video::ITexture* getShadowMapTexture(const irr::u32 resolution, const bool secondary = false);
			...
			inline irr::video::ITexture* getDepthMapTexture()
			{
				return DepthRTT;
			}
			...
			__TORNADO_DLL_EXPORT void addNodeToDepthPass(irr::scene::ISceneNode *node);
			...
			__TORNADO_DLL_EXPORT void removeNodeFromDepthPass(irr::scene::ISceneNode *node);
			...
			__TORNADO_DLL_EXPORT void enableDepthPass(bool enableDepthPass);

			/// Removes shadows from a scene node.
			inline void removeShadowFromNode(irr::scene::ISceneNode* node)
			{
			...
			inline void excludeNodeFromLightingCalculations(irr::scene::ISceneNode* node)
			{
			...
			__TORNADO_DLL_EXPORT void update(irr::video::ITexture* outputTarget = 0);
			...
			__TORNADO_DLL_EXPORT void addShadowToNode(irr::scene::ISceneNode* node, E_FILTER_TYPE filterType = EFT_NONE, E_SHADOW_MODE shadowMode = ESM_BOTH);

			/// Returns the device time divided by 100, for use with the shader callbacks.
			inline irr::f32 getTime() 
			{ 
				return device->getTimer()->getTime() * 0.01f;
			}

			/// Sets the scene clear colour, for when the scene is cleared before smgr->drawAll().
			inline void setClearColour(irr::video::SColor ClearCol)
			{
			...
			*/
			__TORNADO_DLL_EXPORT void addPostProcessingEffect(irr::s32 MaterialType, IPostProcessingRenderCallback* callback = 0);
			...
			inline void setPostProcessingRenderCallback(irr::s32 MaterialType, IPostProcessingRenderCallback* callback = 0)
			{
			...
			inline void removePostProcessingEffect(irr::s32 MaterialType)
			{
				SPostProcessingPair tempPair(MaterialType, 0);
				irr::s32 i = PostProcessingRoutines.binary_search(tempPair);
			...
			__TORNADO_DLL_EXPORT irr::s32 addPostProcessingEffectFromFile(	const irr::core::stringc& filename,
			...
			__TORNADO_DLL_EXPORT void setPostProcessingEffectConstant(	const irr::s32 materialType,
			...
			inline const CScreenQuad& getScreenQuad() 
			{
				return ScreenQuad;
			}

			/// Sets the active scene manager.
			inline void setActiveSceneManager(irr::scene::ISceneManager* smgrIn)
			{
				smgr = smgrIn;
			}

			/// Gets the active scene manager.
			inline irr::scene::ISceneManager* getActiveSceneManager()
			{
				return smgr;
			}
			...
			/// See addPostProcessingEffect for more info.
			inline void setPostProcessingUserTexture(irr::video::ITexture* userTexture)
			{
				ScreenQuad.getMaterial().setTexture(3, userTexture);
			}

			/// Sets the global ambient color for shadowed scene nodes.
			inline void setAmbientColor(irr::video::SColor ambientColour)
			{
				AmbientColour = ambientColour;
			}

			/// Gets the global ambient color.
			inline irr::video::SColor getAmbientColor() const
			{
				return AmbientColour;
			}

			/// Generates a randomized texture composed of uniformly distributed 3 dimensional vectors.
			__TORNADO_DLL_EXPORT irr::video::ITexture* generateRandomVectorTexture(const irr::core::dimension2du& dimensions,
				const irr::core::stringc& name = "randVec");

			/// Sets a new screen render target resolution.
			__TORNADO_DLL_EXPORT void setScreenRenderTargetResolution(const irr::core::dimension2du& resolution);

			/// Returns the device that this EffectHandler was initialized with.
			inline irr::IrrlichtDevice* getIrrlichtDevice() {return device;}
