...
#include "XEffects\ScreenQuadCB.h"
#include "XEffects\DepthShaderCB.h"
#include "XEffects\ShadowShaderCB.h"
#include "XEffects\EffectShaders.h"
...
namespace Tornado 
{
	namespace XEffects
	{
							...
							useVSM(useVSMShadows)
		{
				...
				(driver->getDriverType() == irr::video::EDT_DIRECT3D9 && driver->queryFeature(irr::video::EVDF_PIXEL_SHADER_2_0))))
			{
				...
				Depth = gpu->addHighLevelShaderMaterial(
					...
					sPP.ppShader(SHADOW_PASS_1P[shaderExt]).c_str(), "pixelMain", irr::video::EPST_PS_2_0,
					depthMC, irr::video::EMT_SOLID);

				DepthT = gpu->addHighLevelShaderMaterial(
					sPP.ppShader(SHADOW_PASS_1V[shaderExt]).c_str(), "vertexMain", irr::video::EVST_VS_2_0,
					sPP.ppShader(SHADOW_PASS_1PT[shaderExt]).c_str(), "pixelMain", irr::video::EPST_PS_2_0,
					depthMC, irr::video::EMT_TRANSPARENT_ALPHA_CHANNEL_REF);

				WhiteWash = gpu->addHighLevelShaderMaterial(
					sPP.ppShader(SHADOW_PASS_1V[shaderExt]).c_str(), "vertexMain", irr::video::EVST_VS_2_0,
					sPP.ppShader(WHITE_WASH_P[shaderExt]).c_str(), "pixelMain", irr::video::EPST_PS_2_0,
					depthMC, irr::video::EMT_SOLID);
				...
				WhiteWashTAdd = gpu->addHighLevelShaderMaterial(
					sPP.ppShader(SHADOW_PASS_1V[shaderExt]).c_str(), "vertexMain", irr::video::EVST_VS_2_0,
					sPP.ppShader(WHITE_WASH_P_ADD[shaderExt]).c_str(), "pixelMain", irr::video::EPST_PS_2_0,
					depthMC, irr::video::EMT_TRANSPARENT_ALPHA_CHANNEL);
		...
		SShadowLight* EffectHandler::getShadowLightById(irr::u32 id)
		{
			const irr::u32 LightListSize = LightList.size();
			for(irr::u32 l = 0;l < LightListSize;++l)
			{
		...
		void EffectHandler::enableDepthPass(bool enableDepthPass)
		{
			...
			if(DepthPass && DepthRTT == 0)
				DepthRTT = driver->addRenderTargetTexture(ScreenRTTSize, "depthRTT", use32BitDepth ? irr::video::ECF_G32R32F : irr::video::ECF_G16R16F);
		}
		...
		void EffectHandler::update(irr::video::ITexture* outputTarget)
		{
			...
			if(!ShadowNodeArray.empty() && !LightList.empty())
			{
				...
				for(irr::u32 l = 0;l < LightListSize;++l)
				{
					...
					if(useVSM)
					{
						irr::video::ITexture* currentSecondaryShadowMap = getShadowMapTexture(LightList[l].getShadowMapResolution(), true);
					...
					for(irr::u32 i = 0;i < ShadowNodeArraySize;++i)
					{
						...
						for(irr::u32 m = 0;m < CurrentMaterialCount;++m)
						{
							...
							ShadowNodeArray[i].node->getMaterial(m).MaterialType = (irr::video::E_MATERIAL_TYPE)Shadow[ShadowNodeArray[i].filterType];
			...
			driver->setRenderTarget(PostProcessingRoutinesSize 
				? ScreenRTT : outputTarget, true, true, irr::video::SColor(0x0));
		...
		irr::video::ITexture* EffectHandler::getShadowMapTexture(const irr::u32 resolution, const bool secondary)
		{
			...
			if(shadowMapTexture == 0)
			{
				...
				shadowMapTexture = driver->addRenderTargetTexture(irr::core::dimension2du(resolution, resolution),
					shadowMapName, use32BitDepth ? irr::video::ECF_G32R32F : irr::video::ECF_G16R16F);
			}
			...
			const irr::core::stringc& name)
		{
			...
			for(irr::u32 x = 0;x < dimensions.Width;++x)
			{
				for(irr::u32 y = 0;y < dimensions.Height;++y)
				{
					...
					do {randVec = irr::core::vector3df((irr::f32)rand() / (irr::f32)RAND_MAX, (irr::f32)rand() / (irr::f32)RAND_MAX, (irr::f32)rand() / (irr::f32)RAND_MAX);}
			...
			irr::video::E_MATERIAL_TYPE baseMaterial)
		{
			...
			irr::video::E_VERTEX_SHADER_TYPE VertexLevel = driver->queryFeature(irr::video::EVDF_VERTEX_SHADER_3_0) ? irr::video::EVST_VS_3_0 : irr::video::EVST_VS_2_0;
			irr::video::E_PIXEL_SHADER_TYPE PixelLevel = driver->queryFeature(irr::video::EVDF_PIXEL_SHADER_3_0) ? irr::video::EPST_PS_3_0 : irr::video::EPST_PS_2_0;
			...
			irr::s32 PostMat = gpu->addHighLevelShaderMaterial(
		...
		void EffectHandler::setPostProcessingEffectConstant(const irr::s32 materialType, const irr::core::stringc& name,
															const irr::f32* data, const irr::u32 count)
		{
